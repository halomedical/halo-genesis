...
/**
 * Stream text content with optional inline data parts (images, PDFs, etc.)
 */
export async function* generateContentStreamWithParts(parts: GeminiPart[]): AsyncGenerator<string> {
  const genAI = getGenAI();
  const model = genAI.getGenerativeModel({ model: TEXT_MODEL });
  const result = await withRetry(() =>
    model.generateContentStream(parts, geminiRequestOptions)
  );
  for await (const chunk of result.stream) {
    const text = chunk.text?.();
    if (text) yield text;
